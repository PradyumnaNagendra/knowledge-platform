...
public class ContentValidator {
				...
				if (!isAllRequiredFieldsAvailable(node))
					throw new ClientException(ContentErrorCodeConstants.VALIDATOR_ERROR.name(),
							ContentErrorMessageConstants.MISSING_REQUIRED_FIELDS + " | [Content of Mime-Type '"
									+ node.getMetadata().get(ContentParams.mimeType.name())
	...
	private double getContentPackageFileSizeLimit() {
		...
		if(Platform.config.hasPath(ContentParams.MAX_CONTENT_PACKAGE_FILE_SIZE_LIMIT.name()))
			size = Platform.config.getDouble(ContentParams.MAX_CONTENT_PACKAGE_FILE_SIZE_LIMIT.name());
		return size;
	}
	...
	private boolean isAllRequiredFieldsAvailable(Node node) {
		...
		if (null != node) {
			String name = (String) node.getMetadata().get(ContentParams.name.name());
			...
			if (StringUtils.isNotBlank(mimeType)) {
				...
				case "application/vnd.ekstep.ecml-archive":
					...
					if (StringUtils
							.isNotBlank((String) node.getMetadata().get(ContentParams.body.name()))
							|| StringUtils.isNotBlank(
									(String) node.getMetadata().get(ContentParams.artifactUrl.name())))
						isValid = true;
					else
						throw new ClientException(ContentErrorCodeConstants.VALIDATOR_ERROR.name(),
				...
				case "video/youtube":
					if (StringUtils.isNotBlank(
						...
						Boolean isValidYouTubeUrl = Pattern.matches(YOUTUBE_REGEX,
								node.getMetadata().get(ContentParams.artifactUrl.name()).toString());
				...
				case "application/pdf":
					if (StringUtils.isNotBlank(
						...
						String artifactUrl = (String) node.getMetadata()
								.get(ContentParams.artifactUrl.name());
	...
	public Boolean exceptionChecks(String mimeType, File file) {
		...
		try {
			...
			if (StringUtils.equalsIgnoreCase(mimeType, PDF_MIMETYPE)) {
				if (StringUtils.equalsIgnoreCase(extension, ContentParams.pdf.name()) && file_type.equals("application/pdf")) {
					return true;
				} else {
			...
			if (StringUtils.equalsIgnoreCase(mimeType, EPUB_MIMETYPE)) {
				if (StringUtils.equalsIgnoreCase(extension, ContentParams.epub.name()) && file_type.equals("application/epub+zip")) {
					return true;
				} else {
					throw new ClientException(ContentErrorCodes.INVALID_FILE.name(),
							ContentErrorMessageConstants.INVALID_UPLOADED_FILE_EXTENSION_ERROR
