...
import org.sunbird.content.util.CloudStore;
import org.sunbird.content.util.ContentErrorCodeConstants;
...
public class BaseMimeTypeManager {
	...
	protected Node setNodeStatus(Node node, String status) {
		if (null != node && !StringUtils.isBlank(status)) {
			...
			if (null != metadata) {
				metadata.put(ContentParams.status.name(), status);
	...
	protected boolean hasGivenFile(File file, String checkFile) {
		...
		try {
			...
			throw new ClientException(ErrorCodes.ERR_CONTENT_UPLOAD_FILE.name(), "Invalid zip file");
		} catch (IOException e) {
			throw new ServerException(ErrorCodes.ERR_CONTENT_UPLOAD_FILE.name(), "Error while validating the content");
		}
	...
	private void downloadAppIcon(Node node, String tempFolder) {
		...
		if (StringUtils.isNotBlank(appIcon)) {
			...
			try {
				...
				throw new ServerException("ERR_CONTENT_PUBLISH", "Error while publish the content");
			}
		}
	}
	...
	private void deleteTemp(String sourceFolder) {
		...
		if (!directory.exists()) {
			...
			try {
				...
				throw new ServerException("ERR_CONTENT_PUBLISH", "Error while publishing the content");
			}
		}
	}
	...
	protected Response rePublish(Node node) {
		...
		node = setNodeStatus(node, ContentParams.Live.name());
		String tempFolder = tempFileLocation + File.separator + System.currentTimeMillis() + "_temp";
		File ecarFile = null;
		String artifactUrl = (String) node.getMetadata().get(ContentParams.artifactUrl.name());
		...
		try {
			if (null != ecarFile && ecarFile.exists() && ecarFile.isFile()) {
				...
				node.getMetadata().put(ContentParams.downloadUrl.name(), newName);
			}
	...
	private Response addDataToContentNode(Node node) {
		...
		String bundleFileName = Slug
				.makeSlug((String) node.getMetadata().get(ContentParams.name.name()), true) + "_"
				+ System.currentTimeMillis() + "_" + node.getIdentifier() + "_"
				+ node.getMetadata().get(ContentParams.pkgVersion.name()) + ".ecar";
		...
		node.getMetadata().put(ContentParams.lastPublishedOn.name(), formatCurrentDate());
	...
	protected Response updateContentNode(String contentId, Node node, String url) {
		...
		if (!checkError(updateRes)) {
			if (StringUtils.isNotBlank(url))
				updateRes.put(ContentParams.content_url.name(), url);
			updateRes.put(ContentParams.node_id.name(), contentId);
		}
		return updateRes;
	}
			...
			if (StringUtils.equalsIgnoreCase("Live", status)) {
				...
				if (!searchIds.isEmpty()) {
					...
					if (checkError(searchRes)) {
						...
						List<Object> list = (List<Object>) searchRes.get(ContentParams.contents.name());
...
//		}
	...
	public String[] uploadArtifactToAWS(File uploadedFile, String identifier) {
		...
		try {
			String folder = Platform.config.getString(CONTENT_FOLDER);
			folder = folder + "/" + Slug.makeSlug(identifier, true) + "/" + Platform.config.getString(ARTEFACT_FOLDER);
	...
	protected String getBasePath(String contentId) {
		...
		if (!StringUtils.isBlank(contentId))
			path = tempFileLocation + File.separator + System.currentTimeMillis() + ContentParams._temp.name()
