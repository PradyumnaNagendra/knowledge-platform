...
public class YouTubeUrlUtil {
	...
	public static String getLicense(String videoUrl) {
		...
		if (StringUtils.isBlank(videoId))
			throw new ClientException(ErrorCodes.ERR_INVALID_URL.name(), ERR_MSG);
		...
		if (StringUtils.isBlank(licenceType) && !limitExceeded)
			throw new ClientException(ErrorCodes.ERR_YOUTUBE_LICENSE_VALIDATION.name(), ERR_MSG);

		if (StringUtils.isBlank(licenceType) && limitExceeded)
			throw new ClientException(ErrorCodes.ERR_YOUTUBE_LICENSE_VALIDATION.name(), SERVICE_ERROR);
